 ...
 * @return {void}
 */
function errorLog(msg, fileName) {
    const now = new Date();
    const formattedMsg = `[${(0, date_1.formatDate)(now)}] ${msg}`;
    const coloredFormattedMsg = `${colors_1.COLORS.BgRed}${colors_1.COLORS.FgWhite}` + formattedMsg + colors_1.COLORS.Reset;
    console.log(coloredFormattedMsg);
    if (fileName)
        fs_1.default.appendFileSync(fileName, formattedMsg + '\n');
}
